...
- `players` carries unused legacy `email`/`password` columns from before Clerk was added for auth.
...
- **Legacy `players.email`/`players.password` columns** are dropped entirely. Clerk (`external_id`) is the only auth path; there is no code path that reads or writes these columns today.

## Open items for the implementation plan

- Drizzle schema definitions and migration setup (this project has no ORM or migration tooling yet).
